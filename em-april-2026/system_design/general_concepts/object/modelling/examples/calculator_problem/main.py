...
class Number(Expression):
    ...
    def simplify(self) -> Expression:
        print("simplify not implemented yet !!")
        return self
...
class Variable(Expression):
    ...
    def eval(self, m: VariablesMap) -> float:
        if self.name in m:
            return m[self.name]
        raise ValueError(
            f"Evaluating unknown variable {self.name} with variable-map {m}"
        )
...
class BinaryOperation:
    def execute(
        self, operand1: Expression, operand2: Expression, m: VariablesMap
    ) -> float:
        raise ValueError(
            "Generic operation evaluation not supported for operands {operand1}, {operand2}"
        )
...
def main():
    ...
    expr = BinaryExpression(
        ...
        BinaryExpression(
            plus,
            BinaryExpression(into, Number(3), Variable("x")),
            BinaryExpression(into, Number(2), Variable("y")),
        ),
        Number(-1),
    )
