...
class LRUCache {
   ...
   private:
    lru_elem_type _evictOne() {
        // get front entry from lru
        auto* poppedNode = lru.PopFront();
        // removed poppedNode from index:
        index.erase(poppedNode->Value().key);
        --len;
   ...
   public:
    void put(const K& k, const V& v) {
        ...
        if (iter == index.end()) {
            ...
            auto* removedEntry = lru.RemoveNode(iter->second);
            removedEntry->Value().value = v;
            auto* reinsertedEntry = lru.PushBack(removedEntry);
            iter->second = reinsertedEntry;
        }
        return;
    };
    ...
    value_type get(const key_type& k) {
        ...
        if (iter == index.end()) {
            ...
            auto* removedEntry = lru.RemoveNode(iter->second);
            auto* reinsertedEntry = lru.PushBack(removedEntry);
            return reinsertedEntry->Value().value;
        }
    }
};
...
int main(int argc, char* argv[]) {
    ...
    // cout << "ll = " << ll << endl;
    auto* lru = new LRUCache<int, int>(2);
    lru->put(1, 1);
    cout << "lru = " << lru << endl;
    // cout << " value of get(1) = " << lru->get(1)
    // << ", value of get(2) = " << lru->get(2)
    // << ", value of get(3) = " << lru->get(3) << endl;
    lru->put(2, 3);
    cout << "lru = " << lru << endl;
    lru->put(1, 2);
    cout << "lru = " << lru << endl;
