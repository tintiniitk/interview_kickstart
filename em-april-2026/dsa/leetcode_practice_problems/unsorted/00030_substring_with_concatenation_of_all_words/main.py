    ...
    def findSubstring(self, s: str, words: List[str]) -> List[int]:
        ...
        for phase in range(min(m, n-w+1)):
            ...
            while start < n-w+1:
                qualifies = True
                orig_start = start
                logger.debug(f"  at phase={phase}, start={orig_start}, candidate={s[start:start+w]}, prev_window_had_matched={prev_window_had_matched}, continue_prev_freq_checker={continue_prev_freq_checker}, frequency_checker={frequency_checker}")
                if prev_window_had_matched:
                    ...
                    if new_word != old_word:
                        if new_word not in word_frequency_map:
                            start += w
                            logger.debug(f"  at phase={phase}, start={orig_start}, new_word={new_word} is not in frequency map, so restarting from start={start}")
                        else:
                            reduce_word_in_frequency_checker(new_word)
                            add_word_in_frequency_checker(old_word)
                            logger.debug(f"  at phase={phase}, start={orig_start}, updated frequency_checker to {frequency_checker}")
                            qualifies = False
                            logger.debug(f"  at phase={phase}, start={orig_start}, window is disqualified")
                else:
                    if continue_prev_freq_checker:
                        ...
                        if new_word != old_word:
                            ...
                            logger.debug(f"  at phase={phase}, start={orig_start}, window is just a repeat of the previous window, it is also disqualified")
                            qualifies = False
                    else:
                        reset_frequency_checker() # frequency_checker <= frequency_map
                        logger.debug(f"  at phase={phase}, start={orig_start}, initially, frequency_checker = {frequency_checker}")
                        for j in range(start, start+w, m):
                            ...
                            if word not in word_frequency_map:
                                ...
                                start = j+m
                                logger.debug(f"  at phase={phase}, start={orig_start}, word={word} is not in frequency map, so restarting from start={start}")
                                break
                            if qualifies:
                                if not check_word_in_frequency_checker(word):
                                    logger.debug(f"  at phase={phase}, start={orig_start}, at word={word}, word is not qualified in frequency_checker, so disqualified")
                                    qualifies = False
                            else:
                                reduce_word_in_frequency_checker(word)
                            logger.debug(f"  at phase={phase}, start={orig_start}, at word={word}, frequency_checker -> {frequency_checker}")
                    if start == orig_start and qualifies:
                        if not check_word_frequency_checker():
                            qualifies = False
                            logger.debug(f"  at phase={phase}, start={orig_start}, updated frequency_checker={frequency_checker} is not 0, so disqualified")
                if orig_start == start:
                    if qualifies:
...
def Test(s: str, words: List[str], expected_output: List[int]) -> bool:
    ...
    if not match(expected_output, output):
        if len(s) <= 100 and len(words) <= 100 and len(output) < 100 and len(expected_output) < 100:
            logger.error(f"output(={output}) doesn't match expected_output(={expected_output}) for s={s}, words={words}")
        else:
            logger.error(f"Failed. output doesn't match expected_output")
        return False
    else:
...
Test("ababbaabababbababaababa", ["a", "b", "a", "b"], [0,2,3,4,6,7,8,10,12,13,14,16,18,19])
...
Test("bcabbcaabbccacacbabccacaababcbb", ["c","b","a","c","a","a","a","b","c"], [6,16,17,18,19,20])
